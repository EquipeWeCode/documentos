...
% ----------------------------------------------------------
\subsection{Logs}
Para monitoramento da aplicação em tempo de execução, essencialmente na camada de servidor,
...
Assim, a cada bloco de falha da aplicação um log será colocado para que os problemas sejam identificados,
...
No \gls{backend} os pacotes serão bem divididos, tendo o pacote model para os models, controllers para os controllers e endpoints. 
...
\begin{itemize}
    ...
    \item Aumentar a ortogonalidade do código, diminuindo as dependêcias e o aumentando o desacoplamento e a independência entre os módulos, de modo a de deixa-lo mais fácil de mudar (Easy To Change).
\end{itemize}
...
podendo ser consultados em caso de dúvida em relação a algum método. Esta categoria de teste é chamado teste unitário, que diferente dos testes de integração, que verificam o funcionamento do programa de uma chamada a um endpoint, verificando apenas os serviços externos.

Logo, a construção dos testes, de qualquer natureza é de suma importância para a confecção do projeto no quesito manutenibilidade, seguiremos os principios do \gls{tdd}. Como as ferramentas de teste são específicas para cada linguagem, cada camada fará uso do seu respectivo framework.

O \gls{backend} deverá ser testado com o framework JUnit, já no \gls{frontend} serão feitos com a biblioteca Jest, para a confecção de testes unitários.